...
namespace DBHandler
{
    ...
    public class DBPersonalInformation
    {
        ...
        public Certification CreateOrUpdateCertifications(string userId, object data)
        {
            Helper helper = new Helper();
            JObject pairs = JObject.Parse(data.ToString());
            string title = helper.json.String(pairs, "title");
            Certification certification = entities.Certifications.Where(
                t => t.MemberId == userId && t.CertificateTitle == title).FirstOrDefault();
            if (certification == null)
            {
                certification = new Certification();
                entities.Certifications.Add(certification);
                certification.MemberId = userId;
                certification.CertificateTitle = helper.json.String(pairs, "title");
            }
            certification.GradeRank = helper.json.String(pairs, "grade");
            certification.ValidTill = helper.json.Date(pairs, "validtill");
            certification.DateOfCertification = helper.json.Date(pairs, "certdate");
            certification.Organization = helper.json.String(pairs, "organization");
            certification.AreaOfExpertise = helper.json.String(pairs, "areaexpertise");
        ...
        public Patent CreateOrUpdatePatents(string UserId, object data)
        {
            ...
            if (patent == null)
            {
                ...
                patent.PatentTitle = helper.json.String(pairs, "title");
            }
            patent.PatentDescription = helper.json.String(pairs, "description");
        ...
        public PublicProfile CreateOrUpdatePublicProfile(string UserId, object data)
        {
            Helper helper = new Helper();
            JObject pairs = JObject.Parse(data.ToString());
            PublicProfile publicProfile = entities.PublicProfiles.Where(t => t.MemberId == UserId && t.Title == helper.json.String(pairs, "profileTitle")).FirstOrDefault();
            if (publicProfile == null)
            {
                publicProfile = new PublicProfile();
                entities.PublicProfiles.Add(publicProfile);
                publicProfile.MemberId = UserId;
                publicProfile.Title = helper.json.String(pairs, "title");
            }
        ...
        public Award CreateOrUpdateAwards(string UserId, object data)
        {
            ...
            if (award == null)
            {
                ...
                award.AwardTitle = helper.json.String(pairs, "title");
            }
            award.AwardedBy = helper.json.String(pairs, "awardedBy");
            award.AreaCompititionTitle = helper.json.String(pairs, "competitionTitle");
        ...
        public Copyright CreateOrUpdateCopyRights(string UserId, object data)
        {
            ...
            Copyright copyright = entities.Copyrights.Where(t => t.MemberId == UserId
                           && t.CRTitle == helper.json.String(pairs, "title")).FirstOrDefault();
            if (copyright == null)
            {
                copyright = new Copyright();
                entities.Copyrights.Add(copyright);
                copyright.CRTitle = helper.json.String(pairs, "title");
            }

            copyright.CRDate = helper.json.Date(pairs, "date");
            copyright.CRDescription = helper.json.String(pairs, "description");
        ...
        public Hobby CreateOrUpdateHobbies(string UserId, object data)
        {
            ...
            Hobby hobby = entities.Hobbies.Where(t => t.MemberId == UserId
                                      && t.Hobby1 == helper.json.String(pairs, "title")).FirstOrDefault();
            ...
            if (hobby == null)
            {
                ...
                hobby.Hobby1 = helper.json.String(pairs, "title");
            }

            hobby.Description = helper.json.String(pairs, "description");
            entities.SaveChanges();
